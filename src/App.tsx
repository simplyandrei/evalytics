...
function App() {
  ...
  const editScore = (tabId: number, scoreId: number, label: string, score: number, totalScore: number) => {
    setTabs(tabs.map(tab => {
      if (tab.id === tabId) {
        return {
          ...tab,
          scores: tab.scores.map(s =>
            s.id === scoreId ? { ...s, label, score, totalScore } : s
          )
        };
      }
      return tab;
  ...
  const deleteScore = (tabId: number, scoreId: number) => {
    setTabs(tabs.map(tab => {
      if (tab.id === tabId) {
        return {
          ...tab,
          scores: tab.scores.filter(s => s.id !== scoreId)
        };
      }
      return tab;
    }));
      ...
      <div className="container mt-5 bg-body-secondary rounded-5">
        <div className="container p-5">
          ...
          <div className='justify-content-center'>
            <ScoresTab tabs={tabs} onUpdateTabName={updateTabName} onUpdateTabWeight={updateTabWeight} onDeleteTab={deleteTab} onAddScore={addScore} onEditScore={editScore} onDeleteScore={deleteScore} />
          </div>
          {/* 
